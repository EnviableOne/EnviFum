...
function Get-LastEmailItemAttachment {
    param( 
        [Parameter(Position = 0, Mandatory = $true)] [string]$MailboxName,        
        [Parameter(Position = 2, Mandatory = $false)] [string]$url,
    ...
    Process {
        ...
        $folders = Get-FolderFromPath -MailboxName $MailboxName -FolderPath $FolderPath -service $service  
                   ...
                   $error.clear()
                    ...
                    foreach ($Item in $fiItems.Items) { 
                        foreach($Attachment in $Item.Attachments){
                            if ($Attachment -is [Microsoft.Exchange.WebServices.Data.ItemAttachment]) {
                                $Attachment.Load();
                                foreach($rcp in $Attachment.Item.ToRecipients){
                                    Write-Verbose("To-" + $rcp)
                                }
                            }
                        }
                    }
