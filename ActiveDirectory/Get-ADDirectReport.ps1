				...
				IF ($PSBoundParameters['Recurse'])
				{
					...
					ForEach-Object -Process {
						$_.directreports | ForEach-Object -Process {
							...
							Get-ADUser -Identity $PSItem -Properties title, manager | where samaccountname -notlike "*_ADM" -and distinguishedname -contains "info" | Select-Object -Property @{ Name = "Full Name"; Expression = { $psitem.givenname + " " +$psitem.surname }}, title, @{ Name = "Manager"; Expression = { (Get-Aduser -identity $psitem.manager).givenname + " " +(Get-Aduser -identity $psitem.manager).surname } }
